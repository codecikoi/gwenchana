mixin AppLocale {
  static const String chooseLanguage = 'Choose Language';
  static const String next = 'Next';
  static const String welcomeTo = 'Welcome to';
  static const String settings = 'Settings';
  static const String changeLanguage = 'Change Language';
  static const String login = 'Log in';
  static const String email = 'Email';
  static const String password = 'Password';
  static const String confirmPassword = 'Confirm Password';
  static const String signInWith = 'or Sign in with';
  static const String dontHaveAccount = 'Don\'t have an account?';
  static const String createAccount = 'Create Account';
  static const String forgotPassword = 'Forgot Password?';
  static const String recoverPassword = 'Recover Password';
  static const String reset = 'Reset';
  static const String termsAndConditions =
      "By signing in to Gwenchana, you agree to our Terms of Service and Privacy Policy";
  static const String name = 'Name';
  static const String doHaveAccount = 'Do you have an account';
  static const String signIn = 'Sign in';
  static const String pleaseEnterValidEmail = 'Please enter a valid email';

  // English
  static const Map<String, dynamic> EN = {
    chooseLanguage: 'Choose Language',
    next: 'Next',
    welcomeTo: 'Welcome to',
    settings: 'Settings',
    changeLanguage: 'Change Language',
    login: 'Log in',
    email: 'Email',
    password: 'Password',
    confirmPassword: 'Confirm Password',
    signInWith: 'or Sign in with',
    dontHaveAccount: 'Don\'t have an account?',
    createAccount: 'Create Account',
    forgotPassword: 'Forgot Password?',
    recoverPassword: 'Recover Password',
    reset: 'Reset',
    termsAndConditions:
        'By signing in to Gwenchana, you agree to our Terms of Service and Privacy Policy',
    name: 'Name',
    doHaveAccount: 'Do you have an account',
    signIn: 'Sign in',
    pleaseEnterValidEmail: 'Please enter a valid email',
  };

// Russian (Русский)
  static const Map<String, dynamic> RU = {
    chooseLanguage: 'Выберите язык',
    next: 'Далее',
    welcomeTo: 'Добро пожаловать в',
    settings: 'Настройки',
    changeLanguage: 'Изменить язык',
    login: 'Войти',
    email: 'Электронная почта',
    password: 'Пароль',
    confirmPassword: 'Подтвердите пароль',
    signInWith: 'или Войти через',
    dontHaveAccount: 'Нет аккаунта?',
    createAccount: 'Создать аккаунт',
    forgotPassword: 'Забыли пароль?',
    recoverPassword: 'Восстановить пароль',
    reset: 'Сбросить',
    termsAndConditions:
        'Входя в Gwenchana, вы соглашаетесь с нашими Условиями использования и Политикой конфиденциальности',
    name: 'Имя',
    doHaveAccount: 'У вас есть аккаунт?',
    signIn: 'Войти',
    pleaseEnterValidEmail: 'Пожалуйста, введите действительный email',
  };

// Vietnamese (Tiếng Việt)
  static const Map<String, dynamic> VI = {
    chooseLanguage: 'Chọn ngôn ngữ',
    next: 'Tiếp theo',
    welcomeTo: 'Chào mừng đến',
    settings: 'Cài đặt',
    changeLanguage: 'Thay đổi ngôn ngữ',
    login: 'Đăng nhập',
    email: 'Email',
    password: 'Mật khẩu',
    confirmPassword: 'Xác nhận mật khẩu',
    signInWith: 'hoặc Đăng nhập với',
    dontHaveAccount: 'Chưa có tài khoản?',
    createAccount: 'Tạo tài khoản',
    forgotPassword: 'Quên mật khẩu?',
    recoverPassword: 'Khôi phục mật khẩu',
    reset: 'Đặt lại',
    termsAndConditions:
        'Bằng cách đăng nhập vào Gwenchana, bạn đồng ý với Điều khoản dịch vụ và Chính sách bảo mật của chúng tôi',
    name: 'Tên',
    doHaveAccount: 'Bạn đã có tài khoản?',
    signIn: 'Đăng nhập',
    pleaseEnterValidEmail: 'Vui lòng nhập email hợp lệ',
  };

// Japanese (日本語)
  static const Map<String, dynamic> JA = {
    chooseLanguage: '言語を選択',
    next: '次へ',
    welcomeTo: 'ようこそ',
    settings: '設定',
    changeLanguage: '言語を変更',
    login: 'ログイン',
    email: 'メールアドレス',
    password: 'パスワード',
    confirmPassword: 'パスワードを確認',
    signInWith: 'または次でサインイン',
    dontHaveAccount: 'アカウントをお持ちでない方?',
    createAccount: 'アカウントを作成',
    forgotPassword: 'パスワードを忘れた方?',
    recoverPassword: 'パスワードを回復',
    reset: 'リセット',
    termsAndConditions: 'Gwenchana にサインインすることで、利用規約とプライバシーポリシーに同意したものとみなされます',
    name: '名前',
    doHaveAccount: 'アカウントをお持ちの方?',
    signIn: 'サインイン',
    pleaseEnterValidEmail: '有効なメールアドレスを入力してください',
  };

// French (Français)
  static const Map<String, dynamic> FR = {
    chooseLanguage: 'Choisir la langue',
    next: 'Suivant',
    welcomeTo: 'Bienvenue à',
    settings: 'Paramètres',
    changeLanguage: 'Changer de langue',
    login: 'Se connecter',
    email: 'Email',
    password: 'Mot de passe',
    confirmPassword: 'Confirmer le mot de passe',
    signInWith: 'ou Se connecter avec',
    dontHaveAccount: 'Vous n\'avez pas de compte?',
    createAccount: 'Créer un compte',
    forgotPassword: 'Mot de passe oublié?',
    recoverPassword: 'Récupérer le mot de passe',
    reset: 'Réinitialiser',
    termsAndConditions:
        'En vous connectant à Gwenchana, vous acceptez nos Conditions d\'utilisation et notre Politique de confidentialité',
    name: 'Nom',
    doHaveAccount: 'Vous avez un compte?',
    signIn: 'Se connecter',
    pleaseEnterValidEmail: 'Veuillez entrer une adresse email valide',
  };

// Indonesian (Bahasa Indonesia)
  static const Map<String, dynamic> ID = {
    chooseLanguage: 'Pilih Bahasa',
    next: 'Selanjutnya',
    welcomeTo: 'Selamat datang di',
    settings: 'Pengaturan',
    changeLanguage: 'Ubah Bahasa',
    login: 'Masuk',
    email: 'Email',
    password: 'Kata Sandi',
    confirmPassword: 'Konfirmasi Kata Sandi',
    signInWith: 'atau Masuk dengan',
    dontHaveAccount: 'Belum punya akun?',
    createAccount: 'Buat Akun',
    forgotPassword: 'Lupa Kata Sandi?',
    recoverPassword: 'Pulihkan Kata Sandi',
    reset: 'Atur Ulang',
    termsAndConditions:
        'Dengan masuk ke Gwenchana, Anda menyetujui Ketentuan Layanan dan Kebijakan Privasi kami',
    name: 'Nama',
    doHaveAccount: 'Sudah punya akun?',
    signIn: 'Masuk',
    pleaseEnterValidEmail: 'Silakan masukkan email yang valid',
  };

// Chinese Simplified (简体中文)
  static const Map<String, dynamic> ZH = {
    chooseLanguage: '选择语言',
    next: '下一步',
    welcomeTo: '欢迎来到',
    settings: '设置',
    changeLanguage: '更改语言',
    login: '登录',
    email: '电子邮箱',
    password: '密码',
    confirmPassword: '确认密码',
    signInWith: '或使用以下方式登录',
    dontHaveAccount: '没有账户?',
    createAccount: '创建账户',
    forgotPassword: '忘记密码？',
    recoverPassword: '找回密码',
    reset: '重置',
    termsAndConditions: '登录Gwenchana即表示您同意我们的服务条款和隐私政策',
    name: '姓名',
    doHaveAccount: '已有账户?',
    signIn: '登录',
    pleaseEnterValidEmail: '请输入有效的电子邮箱',
  };

// German (Deutsch)
  static const Map<String, dynamic> DE = {
    chooseLanguage: 'Sprache wählen',
    next: 'Weiter',
    welcomeTo: 'Willkommen bei',
    settings: 'Einstellungen',
    changeLanguage: 'Sprache ändern',
    login: 'Anmelden',
    email: 'E-Mail',
    password: 'Passwort',
    confirmPassword: 'Passwort bestätigen',
    signInWith: 'oder Anmelden mit',
    dontHaveAccount: 'Kein Konto vorhanden?',
    createAccount: 'Konto erstellen',
    forgotPassword: 'Passwort vergessen?',
    recoverPassword: 'Passwort wiederherstellen',
    reset: 'Zurücksetzen',
    termsAndConditions:
        'Mit der Anmeldung bei Gwenchana stimmen Sie unseren Nutzungsbedingungen und Datenschutzbestimmungen zu',
    name: 'Name',
    doHaveAccount: 'Sie haben ein Konto?',
    signIn: 'Anmelden',
    pleaseEnterValidEmail: 'Bitte geben Sie eine gültige E-Mail-Adresse ein',
  };
}
